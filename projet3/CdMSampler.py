...
class CdMSampler:
  ...
  def draw_from_distribution(distribution):
    ...
    for state,proba in distribution.items():
  ...
  def run(self, max_iter):
    ...
    while(iteration<max_iter):
    	...
    	if(self.notify_receive(iteration,state)):
    		break;
    
    	state=self.draw_from_distribution(self.cdm.get_transition_distribution(state))
    	
    	
    	iteration+=1
    
    self.notify_finalize(iteration)
    self.done=True
    
    return self.collect_results(iteration)
